...
const initials = computed(() => { const parts = (a.displayName || a.username).trim().split(/\s+/).filter(Boolean); return parts.length >= 2 ? (parts[0][0] + parts[1][0]).toUpperCase() : (a.displayName || a.username).slice(0, 2).toUpperCase(); });
